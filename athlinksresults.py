...
#----------------------------------------------------------------------
    ...
    for runner in IN:
        ...
        for result in results:
            ...
            racename = unicodedata.normalize('NFKD',course['RaceName']).encode('ascii','ignore')
            coursename = unicodedata.normalize('NFKD',course['Courses'][0]['CourseName']).encode('ascii','ignore')
            outrec['race'] = '{} / {}'.format(racename,coursename)
            outrec['date'] = ftime.epoch2asc(athlinks.gettime(course['RaceDate']))
            outrec['loc'] = unicodedata.normalize('NFKD',course['Home']).encode('ascii','ignore')
            ...
            try:
                agpercent,agresult,agfactor = ag.agegrade(racedateage,resultgen,distmiles,timesecs(resulttime))
                outrec['ag'] = agpercent
